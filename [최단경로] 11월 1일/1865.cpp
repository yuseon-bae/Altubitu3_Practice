//1865 추가제출
#include <iostream>  //c++ 입출력 헤더
#include <tuple>     //tuple 헤더
#include <vector>   //vector 헤더

using namespace std; //std라이브러리 사용
//tuple<int, int, int> 자료형을 tp별칭으로 사용할 것임을 정의
typedef tuple<int, int, int> tp;

//벨만포드 함수
bool bellmanFord(int n,  vector<tp>& edges) {
    //최단거리를 갱신할 dist 벡터를 선언 & 0으로 초기화
    vector<int> dist(n + 1,0);

    for (int i = 1; i <= n; i++) { //모든 정점에서 검사
        bool flag = true; //더 이상 반복문을 실행할 필요가 없는지 확인할 변수
        for (int j = 0; j < edges.size(); j++) { //간선의 수 만큼 
            //s->d인 간선의 가중치가 w
            int s = get<0>(edges[j]); //시작 정점 값
            int d = get<1>(edges[j]); //도착 정점 값
            int w = get<2>(edges[j]); //가중치 값

            int next_weight = dist[s] + w; //d까지의 경로 값

            if (next_weight < dist[d]) { //갱신이 가능한 경우
                flag = false; //갱신이 발생했으므로 계속 반복

                dist[d] = next_weight; //갱신

                if (i == n) { //n번째 단계에서 최단 거리 테이블 갱신 -> 음의 사이클 발생 확인
                    return false; //false 반환 후 종료
                }
            }
        }

        if (flag) { //더 이상 갱신되지 않았다면
            break;  //반복문 종료
        }

    }
   
    return true;  //true 반환
}
/**
* [1865번 웜홀] 벨만 포드 
* 
* 시간이 줄어들면서 출발 위치로 돌아오는 것 = 음의 사이클이 존재. 
* 벨만포드는 출발점이 특정한 한 점일때 가능한 알고리즘.
* 그러나 특정 정점 하나만 확인하면 해당 정점과 단절된 노드가 포함된 음수 사이클을 발견할 수 없음.
* 따라서 1) 모든 정점에 대해서 벨만포드 알고리즘을 돌려서 음수 사이클이 발생하는지 확인 하거나
* 2) cycle의 형성 유무에 대해서 생각해보는 방법이 있다. 
* 사실 초깃값을 INF로 설정하는 이유는 단절된 경우의 경로는 배제시키고 , 어떤 지점까지의 거리를 구하기 위해서이다. 
* 따라서 만약 단순 그래프에서 사이클 유무만 파악할 때는 dist[]초기화를 어떤 값으로 해주어도 상관이 없다.
* 왜냐하면 거리를 구하는 것이 목적이 아닌, 음의 사이클 존재여부만 판별하고 싶기 때문이다.
* (이해가 안됐다면 아래처럼 생각해볼까요?)
* => 모든 정점에 도달할 수 있는 임의의 가짜 정점을 만들어서 시작점을 0으로 하는 한 번의 벨만-포드 연산으로 그래프 전체에 대한 음의 사이클 존재 여부 판단!
* => 이때, 가짜 정점 0에서 모든 정점으로의 거리가 x (아무 값이나 상관없음)인 간선이 있다고 가정하여 dist배열을 x 으로 초기화
* ! 무방향 그래프일때는 방향 그래프 두개로 쪼개서 생각하기!
*/
int main() { //main함수
    //변수 선언
    int tc, n, m, w, s, e, t;
    //테스트케이스의 개수 tc 입력받기
    cin >> tc;
    //테스트케이스의 개수만큼 진행
    while (tc--) {

        //지점의 수 n, 도로의 개수 m, 웜홀의 개수 w 입력
        cin >> n >> m >> w;
        vector<tp> edges; //간선 정보를 저장할 벡터

        //도로 입력
       while(m--){ //도로의 개수 m만큼
           //각 도로의 정보 S, E, T (S와 E: 연결된 지점의 번호, T: 도로를 통해 이동하는데 걸리는 시간) 입력받기
            cin >> s >> e >> t;

            //도로는 무방향이므로
            edges.push_back({ s, e, t }); //양방향 간선 추가
            edges.push_back({ e, s, t }); //양방향 간선 추가
        }
        
        //간선 입력
       while(w--){
            //웜홀의 정보(s: 시작 지점, E: 도착 지점, T: 줄어드는  시간) 입력받기
            cin >> s >> e >> t;

            //웜홀은 단방향
            edges.push_back({ s, e, -t });      
        }


        //연산 및 출력
        if (bellmanFord( n, edges)) { //음의 사이클
            cout << "NO\n"; //NO 출력
        }
        else { //음의 사이클이 아니면
            cout << "YES\n"; //YES 출력

        }
 
    }
}